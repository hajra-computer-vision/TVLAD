...
def c3d_Muhavi(inputs, training, weights=None):
    """
    ...
    net = tf.identity(net, name='fc1')
    net_fc1 = tf.layers.dropout(inputs=net, rate=0.5, training=training)

    net = tf.layers.dense(inputs=net, units=4096, activation=tf.nn.relu,
                          kernel_initializer=tf.constant_initializer(weights[18]),
                          bias_initializer=tf.constant_initializer(weights[19]))
    net = tf.identity(net, name='fc2')
    net_fc2 = tf.layers.dropout(inputs=net, rate=0.5, training=training)

    net = tf.layers.dense(inputs=net_fc2, units=101, activation=None,
...
def c3d_Muhavi_finetune(inputs, training, weights=None):
    """
    ...
    net = tf.layers.dense(inputs=net, units=17, activation=None,
...
def read_train(tr_file):
    ...
    v_paths = [frame_dir + path + '%05d.jpg' % (f + 1) for f in range(start, start + FRAMES)]
...
def read_test(tst_file):
    path, start, cls, NumBatch = tst_file.split(' ')
            
    classname,vidname,b = path.split('/')
    #dir_path = './features/' + classname + '/'
    
    #matFileName = dir_path + vidname
    matFileName = './features/' +vidname
    ...
    return voxel, np.float32(cls), matFileName, NumBatch
...
def demo_test():
    ...
    #classname = 'WaveArms'
    #features_path = './features/' + classname + '/'
    #list_path = './list/' + classname + '.txt'
    list_path = './list/TrainAllClasses.txt'
    ...
    #except OSError:  
        #print ("Creation of the directory %s failed" % features_path)
    #else:  
        #print ("Successfully created the directory %s " % features_path)
    
    weights = sio.loadmat('models/c3d_Muhavi_withoutAct5.mat', squeeze_me=True)['weights']
    ...
    logits, fc1 = c3d_Muhavi(inputs=inputs, training=training, weights=weights)
    #print(tf.shape(fc1))
    ...
    with tf.Session(config=config) as sess:
        ...
        path, start, cls, NumBatch = tst_files[1].split(' ')
        n_sample = int(NumBatch)
       # print(n_sample)
        for epoch in range(1):
            batch_x = np.zeros(shape=(n_sample, FRAMES, CROP_SIZE, CROP_SIZE, CHANNELS), dtype=np.float32)
            batch_y = np.zeros(shape=n_sample, dtype=np.float32)

            
            bidx = 0
            for idx, tst_file in enumerate(tst_files):
                voxel, cls, matFileName, NumBatch = read_test(tst_file)

                matFileName = ''.join(str(e) for e in matFileName)
                batch_x[bidx] = voxel
                batch_y[bidx] = cls
                ...
                if bidx == NumBatch:
                    ...
                    acc, score, fc = sess.run([acc_opt, logits, fc1], feed_dict=feeds)

                    mat = dict()
                    mat['features'] = np.array(fc)
                    sio.savemat(matFileName,mat)
                    print('features saved in: ' + matFileName)

                    # reset batch
                    bidx = 0
                    path, start, cls, NumBatch = tst_files[idx+1].split(' ')
                    n_sample = int(NumBatch)
                    batch_x = np.zeros(shape=(n_sample, FRAMES, CROP_SIZE, CROP_SIZE, CHANNELS), dtype=np.float32)
                    batch_y = np.zeros(shape=n_sample, dtype=np.float32)
...
def demo_finetune():
    ...
    with open('./list/TrainWithoutAct5.txt', 'r') as f:
        lines = f.read().split('\n')
    ...
    # Define the C3D model for UCF101
    inputs = x - tf.constant([96.6], dtype=tf.float32, shape=[1, 1, 1, 1, 1])
    logits = c3d_Muhavi_finetune(inputs=inputs, training=training, weights=weights)
    labels = tf.one_hot(y, 17, name='labels')
    ...
    with tf.Session(config=config) as sess:
        ...
        #saver.restore(sess, os.path.join(model_save_dir, 'my-model-gpu'))
        sess.run(tf.global_variables_initializer())
        ...
        for epoch in range(50):
            ...
            for idx, tr_file in enumerate(tr_files):
                ...
                if (idx + 1) % BATCH_SIZE == 0 or (idx + 1) == n_train:
                    ...
                    _, lss, acc,values = sess.run([train_opt, loss, acc_opt,var_names], feed_dict=feeds)

                    print ('%04d/%04d/%04d, loss: %.3f, acc: %.2f' % (idx / BATCH_SIZE, idx, n_train, lss, acc), ctime())
                    # reset batch
                    bidx = 0
            saver.save(sess,os.path.join(model_save_dir, 'my-model-gpu'), global_step=epoch)
                
        mat = dict()
        mat['weights'] = np.array(values)
        sio.savemat(os.path.join(model_save_dir, 'c3d_Muhavi_withoutAct5'),mat)          
