...
def read_test(tst_file):
    ...
    #frame_dir = './frames/'
    frame_dir = 'drive/My Drive/Colab Notebooks/C3D-tensorflow-master/frames/'
...
def demo_test():
    ...
    with open('drive/My Drive/Colab Notebooks/C3D-tensorflow-master/list/c3d_demo01.txt', 'r') as f:
        lines = f.read().split('\n')
    tst_files = [line for line in lines if len(line) > 0]

    #weights = sio.loadmat('pretrained/c3d_ucf101_tf.mat', squeeze_me=True)['weights']
    weights = sio.loadmat('drive/My Drive/Colab Notebooks/C3D-tensorflow-master/pretrained/c3d_ucf101_tf.mat', squeeze_me=True)['weights']
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(1):
            ...
            mat['features'] = np.vstack(fcc)
            mat['cls'] = np.array(clss)
            mat['vid'] = np.array(vids)

            name = './c3d_demo_01.mat'
